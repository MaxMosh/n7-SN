...
Surechant = kron(ck, [1 zeros(1,Ns -1)]); % surechantillonnage
...
for l = 1:length(phi_tab_rad)
    ...
    for k = 1:length(SNR_lin)
        ...
        if tracer_const == 1
            scatterplot(z_echant); % Constellation 
        ...
        for i=1:length(z_echant)
            if z_echant(i) > 0
                Decision(i) = -sign(z_echant(1));
            else
                Decision(i) = sign(z_echant(1));
            end
        end
        
        % Décodage si le signal a subi un codage par transition
        ak_decode = zeros(size(Decision));
        c_decodage_0 = Decision(1);
        ak_decode(1) = c_decodage_0;
        for i=2:length(z_echant)
            ak_decode(i) = Decision(i)*c_decodage_0;
            c_decodage_0 = Decision(i);
        end
        Decision = ak_decode;
        ...
        %TEB_theorique(k) = 2 * qfunc(abs(cos(phi)) * sqrt(2 * SNR));
        if avec_bruit == 0
            break;
        end
    end

...
hr = ones(1,round(Ns/2)); % Filtre de réception
...
% instants optimaux [Ts/2, Ts]

n0 = 5; % Il suffit de prendre n0 en dehors de l'intervalle pour 
% observer un taux d'erreur binaire non nul
...
Taux_th = qfunc(sqrt(SNR)); % TEB thorique

% Memes opérations pour toutes les valeurs de SNR

Taux_estime_tab2 = zeros(length(SNR_lin), 1);
...
for i=0:8
    ...
    Taux_estime_tab2(i+1) = 1 - sum((Bits_retrouve_bruite == Bits)) / length(Bits);
    Taux_th_tab(i+1) = qfunc(sqrt(SNR_lin(i+1)));
end
...
semilogy(SNR_dB, Taux_estime_tab2, "DisplayName","TEB estimé", 'LineWidth', Epaisseur)

...
procedure routeur_ll is
	...
	Taille : Integer := 0; -- la taille du cache (permet d'éviter d'utiliser une fonction Taille)
	...
	Nbr_Paquets : Integer := 0; -- compte le nombre de paquets routés
	...
	procedure Afficher(Adresse : in T_Adresse_IP; Masque : in T_Adresse_IP ; Interf : in Unbounded_String ; Frequence : in Integer := 0 ; Date : in Integer := 0) is
	...
	procedure Aidez_Moi is -- Affiche un guide d'utilisation
		begin
			...
			Put_Line("-p <fichier>");
			Put_Line("Fin := Clock;Définir le nom du fichier contenant les paquets à router. Par défaut, on utilise le fichier paquets.txt."); New_Line;
			Put_Line("-r <fichier>");
...
begin
		...
		begin
			while not Fini loop
				...
				if Ligne = To_Unbounded_String("table") then
					...
					if Taille_Max /= 0 then -- Il s'agit du routeur avec cache
						Cherche_Route(Cache, Adresse_Paquet, Route_Presente, Route, Interf, Politique, True, Date);
						if Route_Presente then
							...
							if not Route_Presente then
								...
								Put (Sortie, "    " & Interf);
								Ajouter_IP (Cache, Politique, Precision_Cache, Taille, Taille_Max, Adresse_Paquet, Interf, Defaut_Cache, Date);	
							end if;
						end if;
					else -- Il s'agit du routeur simple
					    Cherche_Route(Table_Routage, Adresse_Paquet, Route_Presente, Route, Interf, Politique, False, Date);
					    if not Route_Presente then
								Put (Sortie, "    " & "Aucune route trouvée"); 
						else
		...
		Fin := Clock;
		Duree := Fin - Debut;
		New_Line; Put("Le temps d'exécution : " & Trim(Duration'Image(Duree), Ada.Strings.Left) & " secondes");
exception
	when E : others =>

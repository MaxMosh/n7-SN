...
<!-- TODO : ce chunk r ne fonctionne pas, il faut créer Datalog d'abord -->
```{r}
...
<!-- TODO : remonter ce chunk r avant celui qui précède -->
```{r, echo = FALSE}
Datalog = Data
...
<!-- TODO : subplot --> 
pas fait avec ggplot
montres corrélation en type epci et polluants
```{r}
boxplot(Datalog_$pm10_kg~Datalog_$TypeEPCI)
```
...
<!-- TODO : le fviz ici ne fonctionne pas -->
```{r,  echo = FALSE, include = FALSE}
...
<!-- TODO : les fviz ne fonctionne pas non plus -->
```{r,  echo = FALSE, include = FALSE}
...
peut etre enlever theta
<!-- MODIF : j'ai ajouté indépendants 2 à 2, je sais pas si c'est la formulation correcte par contre -->
<!-- MODIF : j'ai essayé d'aligner aussi -->
$$
\begin{equation*}
(\text{M}_{GL_{1}}) : 
\begin{cases}
\begin{aligned}
\text{dep_met_1000}_i \sim & ~ \mathcal{B}(\pi(x_i)) \text{, indépendants 2 à 2} & \\
\\
...
<!-- TODO : après en soi malgré ce que j'ai dit au dessus la fonction anova semble tout de même avoir une sortie correcte vis-à-vis de la réduction proposée -->
<!-- TODO : au passage le "Chisq" doit pouvoir se justifier avec le fait qu'on fait un test de Wald ou un truc dans le genre -->
<!-- MODIF : écrire la formule D(M_0) - D(M_1) ~ \Khi2 machin bidule -->
<!-- MODIF : le chunk r était en double -->
<!-- MODIF : j'ai interverti les deux modèles dans la fonction ANOVA, ça donne une différence de déviance positive (auparavant négative, tous comme les degrés de libertés qui étaient négatifs) -->
```{r, echo = FALSE, eval = TRUE}
...
Le $\text{pseudoR}^2$ du modèle réduit vaut 0.6490327, c'est une valeur satisfaisante pour fitter notre modèle. (Trouver une autre phrase)

...
En appliquant les différentes méthodes de clustering hiérarchique et en traçant le critère de Calinski-Harabasz et de silhouette pour chacune de ces classifications, on remarque que la plupart des graphes (6 sur les 8) atteignent leur maximum en $K = 2$. On décidera donc de garder deux classes pour la classification hiérarchique. On affiche ci-dessous cette classification hiérarchique en deux classes pour la méthode "complete", dont le maximum était atteint en $K = 2$ pour les deux critères.

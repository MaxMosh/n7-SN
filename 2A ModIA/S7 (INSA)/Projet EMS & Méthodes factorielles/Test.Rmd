...
pas reporté les sorties dans le rapport.
...
Le critère $AIC$ conserve le modèle sans l'interaction annee_inv:TypeEPCI, modèle que l'on note $(\mathcal{M}_{GL_{2}})$.
Le critère $BIC$ conserve le modèle avec les variables nh3_kg, n2o_t, TypeEPCI, nh3_kg:n2o_t, modèle que l'on note $(\mathcal{M}_{GL_{3}})$.

Au passage, on remarque que $(\mathcal{M}_{GL_{3}})$ est sous_modèle de $(\mathcal{M}_{GL_{2}})$.
...
On teste le sous-modèle $(\mathcal{M}_{GL_{2}})$ par rapport au modèle
complet avec interactions $(\mathcal{M}_{GL_{1}})$.

```{r, echo = T, eval = T}
anova(model2, modelcompletinter, test="Chisq")
```

La pvaleur est \>\>0.05, on peut donc conserver le modèle
$(\mathcal{M}_{GL_{2}})$ au risque 5%.
...
Le $\text{pseudoR}^2$ vaut 0.72, ce qui reste une valeur correcte (très
légèrement en dessous du $\text{pseudoR}^2$ du modèle complet avec
interaction).
...
Cependant, le test du sous-modèle $(\mathcal{M}_{GL_{3}})$ vis-à-vis du modèle complet $(\mathcal{M}_{GL_{1}})$ renvoie une pvaleur << 0.05.
On rejette donc le modèle $(\mathcal{M}_{GL_{3}})$ au risque $5\%$.
